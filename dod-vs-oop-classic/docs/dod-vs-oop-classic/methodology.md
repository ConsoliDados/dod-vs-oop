# Methodology — Dod Vs Oop Classic
...
- `architecture/playbook/playbook.md` — **normative rules** (code shape, errors, naming, branching, releases).
- `architecture/playbook/playbook.md` — **stack-specific** rules for ts.

If anything here conflicts with `playbook.md`, the playbook wins. Flag the discrepancy.
...
Promotion is monotonic — every tier contains the previous tier's artefacts. See `playbook.md` §21.

### 1.2 Feature placement mode — where features physically live

| Mode | Layout | Best for |
|---|---|---|
| **A — sprint-bound** (default at small+) | `sprints/sprint-NN/features/<id>-<slug>/` | 3+ contributors, capacity-driven |
| **B — epic-bound** | `epics/<id>-<slug>/features/<id>-<slug>/` | 1–2 contributors, kanban-flow |
| **C — flat** | `features/<id>-<slug>/` | prototype with RPA discipline |

Bidirectional, non-destructive promotion: move folders + update frontmatter. See `playbook.md` §23.

### 1.3 Promotion triggers — when "optional" becomes "required"

| Artefact | Triggered when |
|---|---|
| **SDD** for a context | 3+ ADRs on that context **or** 5+ public use cases **or** 3+ aggregate roots |
...
When a trigger fires, log it in `open-questions.md` (resolved) or the active epic's progress log. See `playbook.md` §22.

---

## 2. The lifecycle

How work flows from idea to release. The diagram is mode-A; modes B and C collapse some boxes (noted below).
...
```
   ...
   │     ▼                                                           │
   │   BACKLOG  (ideation)                                           │
   │     backlogs/<id>-<slug>.md  +  Kanban: Initial→Refining→Ready  │
   │     │                                                           │
   │     │  promote (commit to do it)                                │
   │     ▼                                                           │
   │   ROADMAP  (commitment)                                         │
   │     roadmap/<id>-<slug>.md  +  Kanban: Initial→InProgress→Done  │
   │     card frontmatter `epic:` points at parent epic              │
   │                                                                 │
   │           ◀──── belongs to ──┐                                  │
   │                              │                                  │
   │   EPIC  (multi-sprint group) │                                  │
   │     epics/<id>-<slug>(.md|/)                                    │
   │     type: phase | capability | refactor | infra                 │
   │     Kanban: Planned → Active → Done → Parked                    │
   │     exits_with: [formal completion criteria]                    │
   │     │                                                           │
   │     ▼                                                           │
   │   SPRINT PLANNING (Mode A only; B/C → epic README section)      │
   │     sprints/sprint-NN/planning.md                               │
   │     capacity, justified picks, anticipated risks                │
   │     freezes at sprint start                                     │
   │     │                                                           │
   │     ▼                                                           │
   │   SPRINT (Mode A) / continuous flow (Mode B/C)                  │
   │     sprints/sprint-NN/README.md (or epic README)                │
   │     active_epic, cards in scope, retrospective                  │
   │     │                                                           │
   │     ▼                                                           │
   │   FEATURE  (RPA: Research → Plan → Act)                         │
   │     <container>/features/<id>-<slug>/                           │
   │       feature.md      goal, acceptance, branch                  │
   │       research.md     prior art, tradeoffs, ADR candidates      │
   │       plan.md         tasks with file paths                     │
   │       act.md          live task tracker + retro inline          │
   │     │                                                           │
   │     ├─ may surface ──▶ ADRs (lock-in decisions)                 │
   │     ├─ may surface ──▶ SDDs (tactical design)                   │
   │     │                                                           │
   │     ▼                                                           │
   │   CODE                                                          │
   │     branch  feat/<slug>  off  dev                               │
   │     Conventional Commits → push → PAUSE for human review        │
   │     PR → CI green → squash-merge → dev                          │
   │     │                                                           │
   │     ▼                                                           │
   │   DoD GATE  (AGENTS.md tier-aware checklist)                    │
   │     │                                                           │
   │     ▼                                                           │
   │   ROADMAP CARD → Done; update epic progress log                 │
   │     │                                                           │
   │     ▼  (all epic.exits_with checked?)                           │
   │     │ NO  ──▶ next feature loop                                 │
   │     │                                                           │
   │     ▼ YES                                                       │
   │   EPIC → Done                                                   │
   │     │                                                           │
   │     ▼                                                           │
   │   RELEASE  (playbook §16.4)                                     │
   │     dev → main + semver tag + CHANGELOG entry                   │
   │                                                                 │
   └─────────────────────────────────────────────────────────────────┘
```
...
2. **`docs/dod-vs-oop-classic/architecture/playbook/playbook.md`** — language-agnostic rules. §21 (layered table), §22 (promotion triggers), §23 (modes), §24 (portability).
3. **`docs/dod-vs-oop-classic/architecture/playbook/playbook.md`** — stack-specific examples and idioms.
4. **`docs/dod-vs-oop-classic/architecture/sad.md`** (or `srs+sad.md` at prototype/small) — architecture overview. Get the context map and cross-cutting concerns.
5. **`docs/dod-vs-oop-classic/architecture/srs.md`** (medium+) — requirements contract.
6. **`docs/dod-vs-oop-classic/architecture/adrs/`** — locked-in decisions. Skim titles; read those touching the area you'll work in.
7. **`docs/dod-vs-oop-classic/architecture/sdds/`** (when present) — tactical design per bounded context.
8. **`docs/dod-vs-oop-classic/epics/`** — `_dashboard/board.md` (or flat `epics.md`) shows what's Active. Open the active epic.
9. **`docs/dod-vs-oop-classic/sprints/<active-sprint>/`** (Mode A) **or** the active epic's `features/` (Mode B) — current work.
10. **`docs/dod-vs-oop-classic/open-questions.md`** — unresolved decisions that may block progress.
...
## 4. Day N — the feature loop

When you pick up a card to work on:

### 4.1 Setup

1. Confirm the card is in **Initial** on the roadmap kanban. Move it to **In Progress**.
2. Create the feature folder using the Templater snippet `feature-template.md`:
   - Mode A: container = `sprint-NN`
   - Mode B: container = `<epic-id>-<epic-slug>`
   - Mode C: container = `flat`
3. Branch: `git checkout dev && git pull && git checkout -b feat/<slug>`.

### 4.2 Research (`research.md`)

Read before writing. Document what you considered:

- Internal docs: SAD section for the touched context, related SDDs, prior ADRs.
- External: prior art, library docs, blog posts on similar problems.
- Tradeoffs: at least 2 options, why you'd pick each, your leaning.

If you can't decide between approaches, this is the moment to surface an **ADR candidate** — add it to `open-questions.md` or draft the ADR directly. The plan can't proceed without the decision.

### 4.3 Plan (`plan.md`)

Concrete tasks with file paths. Each task should map to:

- An operation in the relevant SDD (medium+).
- A specific file or set of files.
- Acceptance criteria from `feature.md`.

If a non-trivial decision is locked in here, write the ADR now. Don't ship the plan with an unwritten ADR — that's how decisions get lost.

### 4.4 Act (`act.md`)

Pull tasks from `plan.md` into `act.md` as a `- [ ]` checklist. As work happens:

- Check off completed tasks inline.
- Capture surprises and reshape tasks in place.
- Note punted items (and *why* they got punted).

At feature end, write the closing retro block:
...
**Surprises:** <list>
**Carry forward:** <patterns or follow-ups for the next feature>
```

### 4.5 Code & push

- Conventional Commits per `playbook.md` §16.1: `feat(<scope>): ...`, `fix(<scope>): ...`.
...
Triggered by epic closure, not by calendar. Per `playbook.md` §16.4:
...
- First phase epic (`bootstrap`) → `v0.1.0`.
...
Templater → `epic-template.md` → fill `why`, `outcome`, `scope`, `exits_with`. Kanban: **Planned**. Move to **Active** when a sprint commits to it (Mode A) or when the first feature lands inside (Mode B/C).
...
### 6.6 Create an SDD
Templater → `sdd-template.md` → fill `Aggregates / Use cases / Invariants / Errors / Ports`. Cross-link from `sad.md` and from each ADR that touches the context.
...
Move feature folders. Update each feature's frontmatter `container:` and `mode:` fields. Document the switch in `AGENTS.md` → Methodology section.
...
No if: the context is small and stable.
...
|---|---|
| Look up a code-shape rule | `playbook.md` §1–§20 |
| Decide on tier-conditional artefact | `playbook.md` §21 |
| Decide if an optional artefact must be promoted | `playbook.md` §22 |
| Pick / switch feature placement mode | `playbook.md` §23 |
| Map a concept to Jira / Linear / GH Projects | `playbook.md` §24 |
...
| Find the entry point for a context | `architecture/sdds/sdd-<context>.md` (or the SAD) |
