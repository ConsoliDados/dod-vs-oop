...
Dod Vs Oop Classic — <one-line description>. <Stack summary, e.g. "Turborepo + Bun + Cargo workspace" or "single Rust binary" or "Next.js app">.
...
This project follows the conventions in `docs/dod-vs-oop-classic/architecture/playbook/`. Two declarations frame how the workflow operates here — fill them in and revisit when the project grows or shrinks.
...
Full table, migration effort, and the hybrid (Jira+git) recommendation are in `docs/dod-vs-oop-classic/architecture/playbook/playbook.md` §24. The TL;DR: workflow can migrate to a ticketing tool in 1–3 days if the project ever outgrows markdown; docs always stay in git.
...
Read [[docs/dod-vs-oop-classic/architecture/playbook/playbook.md]] before writing code — it's the long-form version of these rules.
...
3. `docs/dod-vs-oop-classic/architecture/playbook/playbook.md` — **single normative playbook** (combina conventions language-agnostic + stack-specific TS + estilo OOP verbose deste projeto). Não há split base/ts neste projeto — playbook único substitui ambos da template.
4. `docs/dod-vs-oop-classic/architecture/sad.md` (or `srs+sad.md` for prototype tier) — system architecture.
6. `docs/dod-vs-oop-classic/architecture/srs.md` (medium+) — requirements contract.
7. `docs/dod-vs-oop-classic/architecture/adrs/` — locked-in decisions; check before deviating.
8. `docs/dod-vs-oop-classic/architecture/sdds/` (when present) — per-area tactical design.
9. `docs/dod-vs-oop-classic/epics/` — active epic + `exits_with`.
10. `docs/dod-vs-oop-classic/sprints/<active-sprint>/features/<active-feature>/` (Mode A) **or** active epic's `features/` (Mode B) — current work card (research → plan → act).
...
See `docs/dod-vs-oop-classic/architecture/sad.md` for context boundaries; `docs/dod-vs-oop-classic/architecture/sdds/sdd-<context>.md` for the tactical design of each.
...
| `docs/dod-vs-oop-classic/` | Architecture, playbooks, sprint cards. |
...
- [ ] Redlines under `docs/dod-vs-oop-classic/dev-pipeline/redlines/` opened for non-trivial doc changes (SAD/SRS/data-model).
...
Each `<crate>/`, `<package>/`, or `<app>/` has its own `AGENTS.md` with surface-specific conventions, commands, and points of attention. **Read it when you enter the directory.** A template lives at `docs/dod-vs-oop-classic/architecture/playbook/agents/module-AGENTS.template.md`.
