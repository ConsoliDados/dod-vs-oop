...
- The sibling `dod-vs-oop-dod` runs Bun + Elysia + in-memory; the stack difference between the two is a **known confounder**, deliberately accepted and documented in the study `FAQ.md`/`BENCHMARKS.md`.
...
- **(d) Match the DOD stack (Bun + Elysia) to remove the confounder** — rejected for the MVP: it would make the foil unfaithful to real-world verbose DDD. Captured instead as future work (`dod-vs-oop-classic-modern`, see SAD §8).
