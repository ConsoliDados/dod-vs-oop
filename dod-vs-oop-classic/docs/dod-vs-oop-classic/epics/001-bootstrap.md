...
A buildable, tested, documented NestJS project whose `core/` reflects the verbose-canonical (throw-based) DDD style, with the methodology vault (`docs/dod-vs-oop-classic/`) in place and the foundational docs (SRS shared, SAD + ADRs specific) written. `pnpm check` + `pnpm test` green.
...
- Methodology scaffold: `docs/dod-vs-oop-classic/`, root `AGENTS.md` + `CLAUDE.md`, single project `playbook.md`, CI workflow.
