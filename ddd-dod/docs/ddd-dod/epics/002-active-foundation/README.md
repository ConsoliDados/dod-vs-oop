...
- [x] `bun run check` clean; `bun test` green; app boots and shuts down cleanly
- [ ] Epic PR'd into `dev` (medium-tier flow — no milestone branch; AGENTS §Branching / playbook §16.2)
...
- 2026-06-06 — **Branching convention revised → medium-tier flow** (playbook §16.2, **PLAYBOOK-LEARNINGS L-002**). Dropped the `milestone → epic → feature` nesting: **no milestone branch** — `milestone` is a planning grouping + the release marker (`dev → main` tag); **`epic` is the PR unit to `dev`** (`feat→epic` local, **`epic→dev` PR**). Numbered branches matching cards: `feat/<NNN>-<slug>` / `epic/<NNN>-<slug>` / `release/<NNN>-<slug>` (large only). CI/CD: gates by ref, release semantics from Conventional Commits (`commitlint` gate). **Folded to the template immediately** (with consent) — playbook-base §16.2/§16.3/§16.4 updated in both template + project copy. AGENTS §Branching rewritten; DoD now "epic → dev". Branch renamed `epic/active-foundation` → `epic/002-active-foundation`.
