...
- **Shared contract**: the snapshot representation and the "balance = snapshot + delta" semantics are part of the shared SRS, so the `ddd-dod` side adopts the same model (NFR-CORRECT-001 byte-identical JSON).
