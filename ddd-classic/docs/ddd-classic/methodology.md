...
2. **`docs/ddd-classic/architecture/playbook/playbook.md`** — language-agnostic rules. §21 (layered table), §22 (promotion triggers), §23 (modes), §24 (portability).
3. **`docs/ddd-classic/architecture/playbook/playbook.md`** — stack-specific examples and idioms.
4. **`docs/ddd-classic/architecture/sad.md`** (or `srs+sad.md` at prototype/small) — architecture overview. Get the context map and cross-cutting concerns.
5. **`docs/ddd-classic/architecture/srs.md`** (medium+) — requirements contract.
6. **`docs/ddd-classic/architecture/adrs/`** — locked-in decisions. Skim titles; read those touching the area you'll work in.
7. **`docs/ddd-classic/architecture/sdds/`** (when present) — tactical design per bounded context.
8. **`docs/ddd-classic/epics/`** — `_dashboard/board.md` (or flat `epics.md`) shows what's Active. Open the active epic.
9. **`docs/ddd-classic/sprints/<active-sprint>/`** (Mode A) **or** the active epic's `features/` (Mode B) — current work.
10. **`docs/ddd-classic/open-questions.md`** — unresolved decisions that may block progress.
