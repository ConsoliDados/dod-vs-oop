...
- 2026-05-29 — **FEAT-006 (consolidate-balance) implemented** on `feat/consolidate-balance` (off post-PR-#6 `feat/ledger-core`; pending review) — **closes EPIC-002 in scope**. Realizes ADR-0006's append-only `BalanceSnapshot` audit trail. Study's **second cross-aggregate domain service**, `ConsolidateAccountBalance` (pure, no I/O), produces a snapshot from the ledger-recomputed `{ balance, throughSeq }` and the prior latest snapshot; **idempotent** by `throughSeq` monotonicity — no-op when not advancing (gate decision: friendlier to retries than throw). **`LedgerBalanceReader` ACL widened** from `balanceOf → Money` to `balanceAndThroughSeqOf → { balance, throughSeq }` (single query, single source of truth; `CloseAccountUseCase` adapted via one-line destructure). New `AppendBalanceSnapshotRepository` (segregated insert) + `GetLatestBalanceSnapshotRepository` (read latest by `throughSeq DESC`); new `BalanceSnapshotTypeOrmEntity` (append-only schema — no `updatedAt`, no `deletedAt`, no `version`; index on `(accountId, throughSeq)`) + bidirectional mapper. New `BalanceSnapshotDto` + `BalanceSnapshotUseCaseMapper`. New `POST /accounts/:id/consolidations` endpoint (always 200; returns the current snapshot — newly-appended or pre-existing). **The cache is not touched here** — `OnTransactionPostedHandler` owns it; consolidate writes only the snapshot. Tests: `consolidate-account-balance.service.spec` (5 cases — first-snapshot at honest zero, advancing, equal-no-op, stale-no-op, ledger-not-cache structural assertion); `tests/accounts/consolidate-balance.e2e.spec.ts` (5 cases — honest-zero first snapshot, throughSeq advance after postings, no-op on same throughSeq + asOf, 404, **plus a second describe block proving snapshot.balance matches the LEDGER under the FEAT-003 swallow-and-log path even with the cache desynced at 0** — anchors NFR-DATA-001 end-to-end). `pnpm check` clean; `pnpm test` **113 passing** (was 103). All EPIC-002 in-scope exits checked.
