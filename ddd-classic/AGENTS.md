...
**DDD Classic** — **verbose-canonical OOP foil** of the comparative study [DOD + DDD + Small Clean Arch](../README.md). Single NestJS application (Express adapter) + TypeORM + better-sqlite3 (`:memory:`). Implements the **double-entry financial Ledger** domain (Account, Posting, Transaction, Statement, Reconciliation) in Evans/Vernon style — rich AggregateRoot with 300+ lines, `create()` / `buildExisting()` factories, classed validators per entity, **throw on first violation** (does NOT use `Result`), bidirectional mappers, repository segregated per operation, in-memory EventBus with no Outbox.
...
This project follows the conventions in `docs/ddd-classic/architecture/playbook/`. **Important override**: because this is the study's OOP foil, the template's `playbook-base.md` and `playbook-ts.md` were **replaced by a single `playbook.md`** that codifies the verbose-canonical style (throw-based, mappers, segregated repos, no Outbox). The sibling `ddd-dod/` uses the template's original playbooks.
...
Read [[docs/ddd-classic/architecture/playbook/playbook.md]] before writing code — it's the long-form version of these rules plus the verbose-canonical DDD patterns specific to this project.
...
3. `docs/ddd-classic/architecture/playbook/playbook.md` — **single normative playbook** (combines language-agnostic + TS-specific + verbose-canonical OOP style for this project). No base/ts split here.
4. `docs/ddd-classic/architecture/sad.md` — system architecture.
5. `docs/ddd-classic/architecture/srs.md` — requirements contract.
6. `docs/ddd-classic/architecture/adrs/` — locked-in decisions; check before deviating.
7. `docs/ddd-classic/architecture/sdds/` (when present) — per-area tactical design.
8. `docs/ddd-classic/epics/` — active epic + `exits_with` + Mode B features (`epics/<id>/features/<id>/`).
...
See `docs/ddd-classic/architecture/sad.md` for context boundaries; `docs/ddd-classic/architecture/sdds/sdd-<context>.md` for the tactical design of each (once promoted).
...
| `docs/ddd-classic/` | Architecture, playbook, sprint cards, methodology |
...
- **Do NOT use `Result<T, E>`** — that's the DOD side (`ddd-dod/`)
...
Each bounded context (`src/<context>/`) gets its own `AGENTS.md` once it reaches 5+ public items. Template at `docs/ddd-classic/architecture/playbook/agents/module-AGENTS.template.md`. **Read it when you enter the directory.**
